...
class TimeGAN(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.undesire_index = args.undesire_index
        self.desire_index = args.desire_index
        self.lamda_val = args.lamda_val
    ...
    def _generator_forward(self, X, T, Z, gamma=1):
        ...
        if self.command ==  'with_fairness':
            ...
            G_fairness = -1.0 * self.lamda_val * (
                    torch.mean(G[:, self.underpriv_index:] * I[:, self.desire_index:]) / 
                    (X_hat[:, self.S_start_index + self.underpriv_index:].sum()) - 
                    torch.mean(G[:, self.priv_index:] * I[:, self.desire_index:]) / 
                    (X_hat[:, self.S_start_index + self.priv_index:].sum())
                )
        
        # 5. Summation
        G_loss = G_loss_U + gamma * G_loss_U_e + 100 * torch.sqrt(G_loss_S) + 100 * G_loss_V + G_fairness
