...
class BaseModel():
  ...
  def __init__(self, opt, ori_data):
    ...
    self.num_sample = self.opt.size_of_fake_data
  ...
  def generation(self):
    ...
    self.X0, self.T = batch_generator(self.ori_data, self.ori_time, self.opt.batch_size)
    self.Z = random_generator(self.opt.size_of_fake_data, self.opt.z_dim, self.T, self.max_seq_len)
    ...
    val = self.ori_time[0]
    num_sample = int(np.ceil(self.opt.size_of_fake_data/self.max_seq_len))
    T = num_sample *[val]
    generated_data = list()
    for i in range(num_sample):
      temp = generated_data_curr[i, :T[i], :]
      generated_data.append(temp)
    return generated_data

class TimeGAN_Fair(BaseModel):
    """TimeGAN_Fair Class
    """
    ...
    def __init__(self, opt, ori_data, S_start_index, Y_start_index, lamda_val, underpriv_index, priv_index, desire_index):
      super(TimeGAN_Fair, self).__init__(opt, ori_data)
      self.S_start_index = S_start_index
      self.Y_start_index = Y_start_index
      self.lamda_val = lamda_val
      self.underpriv_index = underpriv_index
    ...
    def backward_g(self):
      ...
      G = self.X_hat[:, self.S_start_index:self.S_start_index + 2]
      I = self.X_hat[:, self.Y_start_index:self.Y_start_index + 2]
      self.err_g_f = self.lamda_val * torch.abs(
              torch.mean(G[:, self.underpriv_index:] * I[:, self.desire_index:]) / 
              (self.X_hat[:, self.S_start_index + self.underpriv_index:].sum()) - 
              torch.mean(G[:, self.priv_index:] * I[:, self.desire_index:]) / 
              (self.X_hat[:, self.S_start_index + self.priv_index:].sum())
          )
      self.err_g = self.err_g_U + \
                   self.err_g_U_e * self.opt.w_gamma + \
                   self.err_g_V1 * self.opt.w_g + \
                   self.err_g_V2 * self.opt.w_g + \
                   self.err_g_f * self.opt.w_g_f + \
